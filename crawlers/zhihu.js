...
let loginCookie = '';
// 登陆
const login = ({email, password, captcha, capId}, callback) => {
    ...
    .end((err, res) => {
      ...
      if (body.r == 0) {
        ...
        setCookies.forEach(item => {
          if (item.indexOf('z_c0') == 0) {
            authorization = 'Bearer ' + item.substring(6, item.indexOf(';') - 1);
          }
        })  
        // 设置cookie     
...
const getUserId = callback => {
  ...
  .end((err, res) => {
    if (err) {
      ...
      let $ = cheerio.load(res.text, {decodeEntities: false});
      userId = $('#top-nav-profile-dropdown li')
                .eq(0).find('a').attr('href').substring(8);
...
const getProfile = (callback) => {
  let profileUrl = `https://www.zhihu.com/people/${userId}/activities`;
  superagent
  .get(profileUrl)
  .set('Cookie', loginCookie)
